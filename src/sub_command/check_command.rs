...
impl SubCommandTrait for CheckCommand {
    fn exec<F: FormatterTrait>(&self, _formatter: &F, subcommand_matches_option: Option<&ArgMatches>) -> Result<(), Error> {
        ...
        for (host, error) in error_collection {
            eprintln!("{}", Colour::Red.paint(format!("[ERROR] Host \"{}\": {}", host, prepare_message(error.message()))));
        }
